...
```cpp
void fun() {}     // 表示一个无返回值无参函数

void fun(void) {} // 表示一个无返回值无参函数

void foo(void* param) // 表示一个无返回值任意类型指针参数函数
{
	std::cout << *((string*)param); // 解引用时必须转换为具体的数据类型指针
} 
```
...
![img](.assets/v2-e793cc1f6a147aa3e955345a7da2d57e_1440w-17665039301777.jpg)

## 栈和堆

管理方式：

* 栈是编译器自动管理，在超出作用域时自动销毁释放

* 堆需要手动释放，若不手动释放则在程序结束时由系统释放
...
* 栈是系统提供的数据结构，操作系统在底层提供了对栈的支持
* 堆是由c++函数提供的标准库操作

是否产生碎片：

* 栈在进栈和出栈都有严格的顺序，不会产生碎片
* 堆会频繁的分配和释放，容易造成内存空间的不连续，产生碎片
...
使用 `new` 动态分配内存，使数据保存在堆中

使用 `delete` 手动回收内存



动态分配的内存没有变量名，只能通过指针进行操作
